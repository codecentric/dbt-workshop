{{ config(tags=['unit-test'], enabled=false) }}
...
{% call dbt_unit_testing.test('customers', 'calculate customer lifetime value') %}
  ...
  {% call dbt_unit_testing.mock_ref ("stg_customers", {"input_format": "csv"}) %}
    customer_id::BIGINT,first_name::VARCHAR,last_name::VARCHAR
    1,"Adam",'A.'
    2,"Berta","B."
    3,"Carla","C."
  {% endcall %}
  ...
  {% call dbt_unit_testing.mock_ref ("stg_orders", {"input_format": "csv"}) %}
    order_id::BIGINT,customer_id::BIGINT,order_date::DATE,status::VARCHAR
  ...
  {% call dbt_unit_testing.mock_ref ("stg_payments", {"input_format": "csv"}) %}
    payment_id::BIGINT,order_id::BIGINT,payment_method::VARCHAR,amount::NUMERIC
    1,1,"credit_card",10
    2,2,"gift_card",20
    3,3,"credit_card",30
    4,4,"gift_card",40
  ...
  {% call dbt_unit_testing.expect({"input_format": "csv"}) %}
    customer_id::BIGINT,first_name::VARCHAR,last_name::VARCHAR,most_recent_order::DATE,number_of_orders::BIGINT,customer_lifetime_value::NUMERIC
